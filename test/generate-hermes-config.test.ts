...
describe("agents/hermes/generate-config.ts", () => {
  ...
  it("omits platforms.slack when Slack channel is not enabled", () => {
    ...
    expect(Object.keys(config.platforms)).toEqual(["api_server", "matrix"]);
  });
  ...
  it("includes Matrix in base Hermes platform config with E2EE and auto-join enabled", () => {
    const { config, envFile } = runConfigScript();

    // Matrix base config is always in the generated image — enabled and toolsets are
    // added later by the manifest render at deploy time.
    expect(config.platforms.matrix).toEqual({
      auto_join: true,
      encryption: "optional",
      require_mention: true,
      auto_thread: true,
    ...
    expect(config.platform_toolsets.matrix).toBeUndefined();
    expect(envFile).toContain("MATRIX_ALLOWED_USERS=\n");
  });
