...
export function runOpenshellInstall(deps: RunOpenshellInstallDeps): OpenShellInstallResult {
  ...
  const scriptRaw = path.join(deps.scriptsDir, "install-openshell.sh");
  let spawnBin: string;
  let spawnArgs: string[];
  if (process.platform === "win32") {
    // On Windows: run the install script inside WSL Ubuntu (which has a real Linux env)
    // and create a .cmd shim for the Windows side to call. Path must use WSL /mnt/c/ prefix.
    const wslScript = scriptRaw
      .replace(/\\/g, "/")
      .replace(/^([A-Za-z]):/, (_, d) => `/mnt/${d.toLowerCase()}`);
    spawnBin = "wsl";
    spawnArgs = ["bash", "-c", `sed 's/\\r//' "${wslScript}" | bash`];
  } else {
    spawnBin = "bash";
    spawnArgs = [scriptRaw];
  }
  const result = spawnSync(spawnBin, spawnArgs, {
    cwd: process.platform === "win32" ? undefined : deps.cwd,
    env,
  ...
  if (process.platform === "win32") {
    // Resolve the WSL home dir so the shim points to the freshly installed binary
    const { spawnSync: sp } = require("child_process") as typeof import("node:child_process");
    const wslHomeResult = sp("wsl", ["bash", "-c", "echo $HOME"], { encoding: "utf-8" });
    const wslHome = wslHomeResult.stdout?.trim() || "/home/user";
    const wslOpenshellBin = `${wslHome}/.local/bin/openshell`;

    const winHome = process.env.USERPROFILE || process.env.HOME || "";
    const shimDir = path.join(winHome, ".local", "bin");
    const shimPath = path.join(shimDir, "openshell.cmd");
  ...
  const openshellPath = path.join(localBin, process.platform === "win32" ? "openshell.cmd" : "openshell");
