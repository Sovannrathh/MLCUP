...
const KNOWN_MODEL_SETUP_AGENTS = new Set(["openclaw", "hermes", "worldcup"]);
...
const MODEL_SETUP_EFFECT_KEYS: Record<string, Set<string>> = {
  ...
  worldcup: new Set(["worldcupCompat"]),
};

export type ModelSetupAgent = "worldcup";
...
function validateManifestPayload(payload: unknown, manifestPath: string): ModelSetupManifest {
  if (!isRecord(payload)) throw new Error(`${manifestPath}: manifest must be a JSON object`);
  if (!isNonEmptyString(payload.id)) throw new Error(`${manifestPath}: field 'id' must be a non-empty string`);
  if (!isNonEmptyString(payload.agent)) throw new Error(`${manifestPath}: field 'agent' is required`);
  if (!KNOWN_MODEL_SETUP_AGENTS.has(payload.agent)) throw new Error(`${manifestPath}: unknown agent '${payload.agent}'`);
  if (!isNonEmptyString(payload.description)) throw new Error(`${manifestPath}: field 'description' must be a non-empty string`);
  if (!isRecord(payload.match)) throw new Error(`${manifestPath}: field 'match' must be an object`);
...
function validateMatch(match: Record<string, unknown>, manifestPath: string): void {
  if (Object.keys(match).length === 0) throw new Error(`${manifestPath}: field 'match' must be a non-empty object`);
  ...
  if (unknownKeys.length > 0) throw new Error(`${manifestPath}: unknown match keys: ${unknownKeys.join(", ")}`);
  if (
    match.modelIds !== undefined &&
    (!Array.isArray(match.modelIds) || match.modelIds.length === 0 || !match.modelIds.every(isNonEmptyString))
  ) {
...
function validateSelectedAgentEffects(payload: ModelSetupManifest, manifestPath: string): void {
  ...
  const unknownEffectKeys = Object.keys(payload.effects).filter((key) => !allowedEffectKeys.has(key));
  if (unknownEffectKeys.length > 0) {
    throw new Error(`${manifestPath}: unknown effects for agent '${payload.agent}': ${unknownEffectKeys.join(", ")}`);
  }
  if (payload.agent === "worldcup") {
    const compat = payload.effects.worldcupCompat;
    if (compat !== undefined && !isRecord(compat)) {
      throw new Error(`${manifestPath}: effects.worldcupCompat must be an object`);
    }
  }
}
...
function modelSetupMatches(payload: ModelSetupManifest, context: ModelSetupContext): boolean {
  ...
  if (
    match.modelIds &&
    !new Set(match.modelIds.map((modelId) => modelId.trim().toLowerCase())).has(context.model.trim().toLowerCase())
  ) {
    return false;
  }
  ...
  if (match.baseUrl && trimTrailingSlash(context.baseUrl) !== trimTrailingSlash(match.baseUrl)) return false;
  return true;
}
